...
controller.show = function(request, reply){
  ...
  db.find({}, function(err,docs) {
    reply.view('publications.hbs',{ activep: true, publications: docs,  user: user}, { layout: 'logged' });
  });
};

controller.new = function(request, reply){
  var user = request.auth.credentials;
  return reply.view('createPublication.hbs', { activep: true, user: user}, {layout: 'logged'});
};
...
controller.create = function(request, reply){
  ...
  if (!request.payload.title || !request.payload.file)
    return reply.view('createPublication.hbs', {activep: true, message: "Please provide some information about the document", user: user}, {layout: 'logged'});
  else {
...
controller.get = function(request, reply){
  ...
  db.findOne({ _id: publicationId },{}, function(err,docs) {
    ...
    reply.view('publicationDetails.hbs',{ activep: true, publication: docs,  user: user.username}, { layout: 'logged' });
  });
};
