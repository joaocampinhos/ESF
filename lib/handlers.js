...
function profile(request, reply) {
  ...
  db.find({ username: request.params.username }, function(err, docs) {
    var profile = docs[0];
    if (!profile) {
      reply.view('profile.hbs', { message: 'User not found', user: user.username }, {layout: 'logged'});
    }
    else {
      reply.view('profile.hbs', { title: 'My home page', profile: profile, user: user.username }, { layout: 'logged' });
    }
  });
};
