...
function pub(request, reply){
  ...
  db.find({}, function(err,docs) {
    var publications = docs.map(function(el) {
      var access=el.usersWithPermission.filter(function(el2){
            return el2.id==user._id;
        });
      var pubEl= {
        id:el._id,
        title: el.title,
        ...
        hasAccess: access.length>0?access[0].isConfirmed:false
      };
      return pubEl;
    });
    console.log(publications);
    reply.view('publications.hbs',{ publications: publications,  user: user.username}, { layout: 'logged' });
  });
};
...
function createpub(request, reply){
  ...
  if (request.method === 'get') {
    return reply.view('createPublication.hbs', {}, {layout: 'logged'});
  }
  if (request.method === 'post') {
    console.log(request.payload);
    //O if mais feio de sempre?
...
function askAccess(request, reply){
  var db = request.server.plugins['hapi-nedb'].db.collection('publications');
  var user = request.auth.credentials;
  var publicationId = request.params.id;
  db.update({ _id: publicationId },{$addToSet: {usersWithPermission:{id:user._id,isConfirmed:false}}}, function(err,docs) {
    reply.redirect('/publications');
 });
};